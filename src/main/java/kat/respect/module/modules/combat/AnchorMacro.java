...
public final class AnchorMacro extends Module implements TickListener, GameRenderListener {
   private final KeybindSetting triggerKey = new KeybindSetting("Trigger Key", 71).setDescription("Keybind that activates anchor macro");
   private final ModeSetting<AnchorMacro.Mode> mode = new ModeSetting<>("Mode", AnchorMacro.Mode.SILENT, AnchorMacro.Mode.class)
   ...
   private final BooleanSetting blatant = new BooleanSetting("Blatant", false)
      ...
      .setVisible(() -> this.mode.isMode(AnchorMacro.Mode.SILENT));
   ...
   private AnchorMacro.Step currentStep = AnchorMacro.Step.IDLE;
   ...
   public AnchorMacro() {
      super("Anchor Macro", "GrimAC-bypassing fast anchor macro without self protection", -1, Category.COMBAT);
      this.addSettings(this.triggerKey, this.mode, this.blatant, this.switchDelay, this.totemSlot, this.range, this.rotationSpeed, this.checkLoot);
   }
   ...
   public void onGameRender(GameRenderListener.GameRenderEvent event) {
      if (this.mc.field_1724 != null && this.isActive && this.mode.isMode(AnchorMacro.Mode.CAMERA) && this.hasRotationTarget) {
      ...
      if (this.mc.field_1755 == null && this.mc.field_1724 != null && this.mc.field_1687 != null) {
         if (this.finishCooldown > 0) {
            ...
            if (!this.isActive && KeyUtils.isKeyPressed(this.triggerKey.getKey())) {
               this.isActive = true;
               this.currentStep = AnchorMacro.Step.FIND_TARGET;
               this.delayCounter = 0;
            ...
            if (this.isActive) {
               ...
               boolean isBlatant = this.blatant.getValue() && this.mode.isMode(AnchorMacro.Mode.SILENT);
               ...
               while (this.isActive && stepsExecuted < maxStepsPerTick) {
                  ...
                  AnchorMacro.Step prevStep = this.currentStep;
                  ...
                  stepsExecuted++;
                  if (this.currentStep == prevStep || this.currentStep == AnchorMacro.Step.IDLE) {
                     break;
                  }
               }
            }
         ...
         if (hitState.method_27852(class_2246.field_23152)) {
            ...
            if (this.mc.field_1724.method_73189().method_1022(class_243.method_24953(this.targetAnchorPos)) > this.range.getValue()) {
               ...
               if (BlockUtils.isAnchorCharged(this.targetAnchorPos)) {
                  this.advanceTo(AnchorMacro.Step.SWAP_TOTEM);
               } else {
                  this.advanceTo(AnchorMacro.Step.CHARGE_ANCHOR);
               }
            }
         } else {
            ...
            if (this.mc.field_1724.method_73189().method_1022(class_243.method_24953(this.targetAnchorPos)) > this.range.getValue()) {
               ...
               this.advanceTo(AnchorMacro.Step.PLACE_ANCHOR);
            }
         }
      }
   }
   ...
   private void executeSwapTotem() {
      ...
      this.advanceTo(AnchorMacro.Step.EXPLODE);
   }
   ...
   private void executeExplode() {
      if (this.targetAnchorPos == null) {
         ...
         if (this.isRotationClose()) {
            ...
            this.advanceTo(AnchorMacro.Step.DONE);
         }
      }
   }

   private void advanceTo(AnchorMacro.Step next) {
   ...
   private void fullReset() {
      this.isActive = false;
      this.currentStep = AnchorMacro.Step.IDLE;
      this.delayCounter = 0;
   ...
   private boolean isRotationClose() {
      if (this.blatant.getValue() && this.mode.isMode(AnchorMacro.Mode.SILENT)) {
         return true;
      }
      ...
      float currentYaw;
      float currentPitch;
      if (this.mode.isMode(AnchorMacro.Mode.SILENT) && RotationHandler.isSilentActive()) {
   ...
   private class_243 calculateMinimalAngleTarget(class_2338 pos, class_2350 side) {
      if (this.mc.field_1724 != null && this.mc.field_1687 != null) {
         float currentYaw = this.mode.isMode(AnchorMacro.Mode.SILENT) && RotationHandler.isSilentActive()
            ? RotationHandler.getSilentYaw()
            : this.mc.field_1724.method_36454();
         float currentPitch = this.mode.isMode(AnchorMacro.Mode.SILENT) && RotationHandler.isSilentActive()
   ...
   private void performBlockInteraction(class_3965 hit) {
      if (this.mc.field_1724 != null && this.mc.field_1761 != null && hit != null) {
         if (this.mode.isMode(AnchorMacro.Mode.CAMERA)) {
